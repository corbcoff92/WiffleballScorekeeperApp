...
{      
    ...
    public String getDisplayString()
    {
        ...
        // Print scoreboard
        displayText +=  "\n" + String.format("%d inning game", NUM_INNINGS) + "\n";
        displayText += String.format("%-" + NAME_PADDING + "s   R  H  W  |  %c  |", (isTopOfInning ? "TOP " : "BOT ") + inning, second) + "\n";
        displayText += String.format("%-" +  NAME_PADDING + "s   %-2d %-2d %-2d |%c   %c|", awayTeam.name, awayTeam.getRuns(), awayTeam.hits, awayTeam.walks, third, first) + "\n";
        displayText += String.format("%-" +  NAME_PADDING + "s   %-2d %-2d %-2d |  O  |", homeTeam.name, homeTeam.getRuns(), homeTeam.hits, homeTeam.walks, third, first) + "\n";
        displayText += count.getDisplayString() + ", " + outs + (outs != 1 ? " outs" : " out")  + "\n";
        displayText += message;

        return displayText;
    } 

    public boolean isTopInning() {return isTopOfInning;}
    public int getInning() {return inning;}
    public int getBalls() {return count.balls;}
    public int getStrikes() {return count.strikes;}
    public int getOuts() {return outs;}
    public int getHomeRuns() {return homeTeam.getRuns();}
    public int getHomeHits() {return homeTeam.hits;}
    public int getHomeWalks() {return homeTeam.walks;}
    public int getAwayRuns() {return awayTeam.getRuns();}
    public int getAwayHits() {return awayTeam.hits;}
    public int getAwayWalks() {return awayTeam.walks;}
    public boolean isRunnerOnBase(int base) {return runners.contains(base);}
